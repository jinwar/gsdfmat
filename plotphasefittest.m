...
for i=1:m
	if data(i,1) < stalalim(2) && data(i,1) > stalalim(1) ...
		...
		coor(j,2)=data(i,2);
		coor(j,3)=data(i,3);
		coor(j,4)=distance(coor(j,1),coor(j,2),evla,evlo)*111/4+rand*1;
		j=j+1;
	end
end
...
%        if abs(testdz(i,j)-4) < 0.05 && abs(testaspect(i,j)-azi(i,j)) < 1
			aspectch(i,j)=testaspect(i,j)-azi(i,j);
%		else
%			aspectch(i,j)=nan;
%			dz(i,j)=nan;
%        end
